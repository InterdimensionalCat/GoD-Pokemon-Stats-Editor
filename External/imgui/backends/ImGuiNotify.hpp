...
#define NOTIFY_FADE_OUT_TIME				500			// Fade out duration
#define NOTIFY_FADE_IN_TIME				    200			// Fade in duration
...
public:
    ...
    inline ImGuiToastPhase getPhase()
    {
        ...
        if (elapsed > NOTIFY_FADE_OUT_TIME + this->dismissTime + NOTIFY_FADE_OUT_TIME)
        {
            return ImGuiToastPhase::Expired;
        } else 
        if (elapsed > NOTIFY_FADE_OUT_TIME + this->dismissTime)
        {
            return ImGuiToastPhase::FadeOut;
        } else 
        if (elapsed > NOTIFY_FADE_OUT_TIME)
        {
    ...
    inline float getFadePercent()
    {
        ...
        {
            return ((float)elapsed / (float)NOTIFY_FADE_IN_TIME) * NOTIFY_OPACITY;
        } else 
        if (phase == ImGuiToastPhase::FadeOut)
        {
            return (1.f - (((float)elapsed - (float)NOTIFY_FADE_OUT_TIME - (float)this->dismissTime) / (float)NOTIFY_FADE_OUT_TIME)) * NOTIFY_OPACITY;
        }
